...
export async function GET(
  ...
  if (isNaN(otherId)) return Response.json({ error: 'Invalid conversation ID' }, { status: 400 })

  // Support polling: only return messages newer than `after` param
  const url = new URL(req.url)
  const after = url.searchParams.get('after')

  try {
    const result = await pool.query(
      `SELECT m.id, m.sender_id, m.recipient_id, m.message_text,
       ...
       ${after ? 'AND m.created_at > $3' : ''}
       ORDER BY m.created_at ASC`,
      after ? [payload.userId, otherId, after] : [payload.userId, otherId]
    )
